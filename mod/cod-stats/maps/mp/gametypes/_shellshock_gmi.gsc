...
DoShellShock(weapon, sMeansOfDeath, sHitLoc, iDamage)
{
	...
	{		
		...
		{
			self thread gmiPlayerShellShocked(weapon, sMeansOfDeath, sHitLoc, iDamage);
		}
		...
		{
			self thread gmiPlayerShellShocked(weapon, sMeansOfDeath, sHitLoc, iDamage, "melee");
		}
	}
}
...
// 	Shell shock effect for nades and rockets in GMI multiplayer
// ----------------------------------------------------------------------------------
gmiPlayerShellShocked(weapon, sMeansOfDeath, sHitLoc, dmg, customShellShock)
{
	time = 0;
    ...
    // Log the shell shock event
	lpselfguid = self getGuid();
	lpselfnum = self getEntityNumber();
	lpselfteam = self.pers["team"];
	lpselfname = self.name;
    logPrint("Shock;" + lpselfguid + ";" + lpselfnum + ";" + lpselfteam + ";" + lpselfname + ";" + weapon + ";" + time + ";" + sMeansOfDeath + ";" + sHitLoc + "\n");
