...
{
////////// Added by AWE //////////
	level.allow_ft_allies = getCvar("scr_allow_ft_allies");
	if(level.allow_ft_allies == "")
		level.allow_ft_allies = "0";
	setCvar("scr_allow_ft_allies", level.allow_ft_allies);
	setCvar("ui_allow_ft_allies", level.allow_ft_allies);
	makeCvarServerInfo("ui_allow_ft_allies", "1");

	level.allow_rl_allies = getCvar("scr_allow_rl_allies");
	if(level.allow_rl_allies == "")
		level.allow_rl_allies = "0";
	setCvar("scr_allow_rl_allies", level.allow_rl_allies);
	setCvar("ui_allow_rl_allies", level.allow_rl_allies);
	makeCvarServerInfo("ui_allow_rl_allies", "1");

	level.allow_ft_axis = getCvar("scr_allow_ft_axis");
	if(level.allow_ft_axis == "")
		level.allow_ft_axis = "0";
	setCvar("scr_allow_ft_axis", level.allow_ft_axis);
	setCvar("ui_allow_ft_axis", level.allow_ft_axis);
	makeCvarServerInfo("ui_allow_ft_axis", "1");

	level.allow_rl_axis = getCvar("scr_allow_rl_axis");
	if(level.allow_rl_axis == "")
		level.allow_rl_axis = "0";
	setCvar("scr_allow_rl_axis", level.allow_rl_axis);
	setCvar("ui_allow_rl_axis", level.allow_rl_axis);
	makeCvarServerInfo("ui_allow_rl_axis", "1");

/*	level.allow_uoweapons = getCvar("scr_allow_uoweapons");
	if(level.allow_uoweapons == "")
		level.allow_uoweapons = "0";
	setCvar("scr_allow_uoweapons", level.allow_uoweapons);
	setCvar("ui_allow_uoweapons", level.allow_uoweapons);
	makeCvarServerInfo("ui_allow_uoweapons", "1");*/
...
updateWeaponCvars()
{
	...
	{
////////// Added by AWE //////////
		scr_allow_ft_allies = getCvar("scr_allow_ft_allies");
		if(level.allow_ft_allies != scr_allow_ft_allies)
		{
			level.allow_ft_allies = scr_allow_ft_allies;
			setCvar("ui_allow_ft_allies", level.allow_ft_allies);
		}

		scr_allow_rl_allies = getCvar("scr_allow_rl_allies");
		if(level.allow_rl_allies != scr_allow_rl_allies)
		{
			level.allow_rl_allies = scr_allow_rl_allies;
			setCvar("ui_allow_rl_allies", level.allow_rl_allies);
		}

		scr_allow_ft_axis = getCvar("scr_allow_ft_axis");
		if(level.allow_ft_axis != scr_allow_ft_axis)
		{
			level.allow_ft_axis = scr_allow_ft_axis;
			setCvar("ui_allow_ft_axis", level.allow_ft_axis);
		}

		scr_allow_rl_axis = getCvar("scr_allow_rl_axis");
		if(level.allow_rl_axis != scr_allow_rl_axis)
		{
			level.allow_rl_axis = scr_allow_rl_axis;
			setCvar("ui_allow_rl_axis", level.allow_rl_axis);
		}

/*		scr_allow_uoweapons = getCvar("scr_allow_uoweapons");
		if(level.allow_uoweapons != scr_allow_uoweapons)
		{
			level.allow_uoweapons = scr_allow_uoweapons;
			setCvar("ui_allow_uoweapons", level.allow_uoweapons);
		}*/
...
/////////////// Added by AWE ////////////
				...
				if(!getcvar("scr_allow_ft_" + self.pers["team"]))
				{
					self iprintln("Flamethrower is a restricted weapon");
					response = "restricted";
				}
				break;

                  case "bazooka_mp":
				if(!getcvar("scr_allow_rl_"  + self.pers["team"]))
				{
					self iprintln("Bazooka is a restricted weapon");
					response = "restricted";
				}
				break;
...
/////////////// Added by AWE ////////////
			...
			case "sten_silenced_mp":
				...
				{
					self iprintln("Silenced Sten is a restriced weapon");
					response = "restricted";
				}
				break;
...
////////////// Added by AWE /////////////
				...
				if(!getcvar("scr_allow_rl_" + self.pers["team"]))
				{
					self iprintln("Panzerschreck is a restricted weapon");
					response = "restricted";
				}
				break;
...
////////// Added by AWE //////////
		if(!level.awe_disable && !(isdefined(level.roundstarted) && !level.roundstarted) && isAlive(self) && self.sessionstate == "playing")
		{
			if(response == "AWE DW")
			{
				if(level.awe_vsaydropweapon)
					self dropitem(self getcurrentweapon());
				else
					self iprintln("VSAY Weapon drop disabled on the server.");
				continue;
			}
			else if(response == "AWE DH")
			{
				if(level.awe_vsaydrophealth)
	 				self maps\mp\gametypes\_awe::dropHealth(true);
				else
					self iprintln("VSAY Health drop disabled on the server.");
				continue;
			}
		}				
//////////////////////////////////
...
/////// Added by AWE ////////				
	...
	case "sten_silenced_mp":
		weaponname = &"GMI_WEAPON_STEN_SILENCED";
		break;
