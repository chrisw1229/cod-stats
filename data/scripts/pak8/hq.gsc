...
Callback_StartGameType()
{
	...
	precacheModel("xmodel/objective_german_field_radio");
...
hq_setup()
{
	...
	if(getcvarint("scr_hq_radios") > level.radio.size)
	{
		setcvar("scr_hq_radios", level.radio.size);
		level.maxRadios = getcvarfloat("scr_hq_radios");
	}
...
hq_obj_think(radio)
{
	...
	{
		//Both teams have a radio so remove other radios from the compass
		for ( i=0 ; i<level.radio.size ; i++ )
		{
			if (level.radio[i].team == "none")
			{
				level.radio[i] hide();
				level.radio[i].hidden = true;
				objective_delete(level.radio[i].objnum);
			}
		}
	}
	else
	{
		...
		for ( i=0 ; i<(level.maxRadios - NeutralRadios) ; i++ )
		{
			randnum = randomint(level.radio.size);
			if (isdefined (radio))
			{
				while ( (level.radio[randnum].team != "none") || (level.radio[randnum].hidden == false) || (level.radio[randnum] == radio))
					randnum = randomint(level.radio.size); // It picked a radio already in use or it picked the same location it was just at so pick again
			}
			else
			{
				while ( (level.radio[randnum].team != "none") || (level.radio[randnum].hidden == false))
					randnum = randomint(level.radio.size); // It picked a radio already in use so pick again
			}
			
			objective_add(level.radio[randnum].objnum, "current", level.radio[randnum].origin, game["radio_prespawn"]);
	...
	while (!level.mapended)
	{
		...
		if (!radio.hidden)
		{
			...
			for(i = 0; i < players.size; i++)
			{
				...
				{
					if ( ((distance(players[i].origin,radio.origin)) <= level.radioradius) && (distance((0,0,players[i].origin[2]),(0,0,radio.origin[2])) <= level.zradioradius) )
					{
						...
						if ( (!isdefined (players[i].radioicon)) || (!isdefined (players[i].radioicon[radio.objnum])) )
						{
							players[i].radioicon[radio.objnum] = newClientHudElem(players[i]);
							players[i].radioicon[radio.objnum].alignX = "center";
							players[i].radioicon[radio.objnum].alignY = "middle";
							players[i].radioicon[radio.objnum].x = 600;
							players[i].radioicon[radio.objnum].y = 390;
							players[i].radioicon[radio.objnum] setShader("gfx/hud/hud@field_radio.tga", 40, 32);
						}
					...
					else if ( (isdefined (players[i].radioicon)) && (isdefined (players[i].radioicon[radio.objnum])) )
					{
						if ( (isdefined (players[i].radioicon)) || (isdefined (players[i].radioicon[radio.objnum])) )
							players[i].radioicon[radio.objnum] destroy();
						if(isdefined(players[i].progressbar_capture))
...
hq_radio_capture(radio, team)
{
	...
	if (team == "none")
	{
		...
		radio setmodel ("xmodel/objective_german_field_radio");
	...
	else
	{
		...
		radio setmodel ("xmodel/german_field_radio");
	...
	objective_team(radio.objnum, "none");
	...
	for ( i=0 ; i<level.radio.size ; i++ )
	{
		if (level.radio[i].hidden == true)
			continue;
		if (level.radio[i].team == "none")
			objective_team(level.radio[i].objnum, objteam);
	}
...
hq_removeall_hudelems(player)
{
	if (isdefined (self))
	{
		for ( i=0 ; i<level.radio.size ; i++ )
		{
			if ( (isdefined (player.radioicon)) && (isdefined (player.radioicon[level.radio[i].objnum])) )
